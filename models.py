...
# from aiogram import types #OLD AIOGRAM
...
from utils import get_formatted_username_or_id, PostMode, get_formatted_username_or_id_chat
...
def update_db():
    try:
        ...
        migrate(
            migrator.add_column('user', 'has_sudo', BooleanField(default=False)),
            migrator.add_column('user', 'has_banned', BooleanField(default=False))
        )
    ...
    def get_or_create(user: types.User):
        try:
            ...
            result = User.create(
                ...
                has_sudo = False,
                has_banned = False)
            logger.info('novo usuário ' + get_formatted_username_or_id(user) + ' foi salvo no banco de dados')
        return result

    def refresh(self, user: types.User):
        # print('refreshing:',user.id)
    ...
    def get_or_create(group: types.Chat):
        try:
            ...
            result = Group.create(
                ...
                first_interaction_time = datetime.utcnow(),
                last_interaction_time = datetime.utcnow())
            logger.info('novo usuário ' + get_formatted_username_or_id_chat(group) + ' foi salvo no banco de dados')
        return result
