...
# from aiogram import Bot, Dispatcher, executor, types #OLD AIOGRAM
...
from models import User, Post, Group, PostMode
from resources import Resources
from utils import get_formatted_username_or_id, timeFormatter
...
# bot = Bot(token = os.environ['API_TOKEN']) #OLD AIOGRAM
bot = AsyncTeleBot(os.environ['API_TOKEN'])

# dp = Dispatcher(bot) #OLD AIOGRAM
scheduler = AsyncIOScheduler()
channelStatusId = int(os.environ['channelStatusId'])

# Mensagem padrão para broadcast (se for alterar, manter os {} que serve para identificar users e grupos)
...
async def set_my_configs():
    for locale in locales.locales:
        # print(locale)
        try:
            # print(locales[locale].commands_start, locales[locale].commands_help)
            await bot.set_my_commands([types.BotCommand("/start", locales[locale].commands_start),types.BotCommand("/help", locales[locale].commands_help)],language_code=locale, scope=types.BotCommandScopeAllPrivateChats())
        except Exception as ex:
            logger.error(ex)
        try:
            # print(locales[locale].bot_name)
            await bot.set_my_name(name=locales[locale].bot_name, language_code=locale)
        except Exception as ex:
            logger.error(ex)
        try:
            # print(locales[locale].description)
            await bot.set_my_description(description=locales[locale].description, language_code=locale)
        except Exception as ex:
            logger.error(ex)
        try:
            # print(locales[locale].short_description)
            await bot.set_my_short_description(short_description=locales[locale].short_description, language_code=locale)
        except Exception as ex:
            logger.error(ex)
    for admin in admins:
        try:
...
scheduler.add_job(sendStatus, 'cron', hour=12, minute=0, second=5, timezone='America/Sao_Paulo')
scheduler.start()
...
def create_post(author: User, content: str, scope: set = ()):
    try:
        ...
        logger.info('#' + str(result.get_id()) + ' foi criado por ' + get_formatted_username_or_id(author))
        return result
    except Exception as e:
        logger.error('nova postagem não pode ser criada por ' + get_formatted_username_or_id(author))
        logger.error(e)
...
def list_all_users_db():
    # return User.select(User.user_id).where(User.has_dialog)
        ...
        for _try in range(retry):
            try:
                if message.content_type == 'text':
                    await bot.send_message(user,message.text, reply_markup=keyboard, disable_web_page_preview=True)
                elif message.content_type == 'photo':
                    if message.caption:
                        await bot.send_photo(user, message.photo[-1].file_id, message.caption,reply_markup=keyboard)
                    else:
                        await bot.send_photo(user, message.photo[-1].file_id,reply_markup=keyboard)
                elif message.content_type == 'audio':
                    if message.caption:
                        await bot.send_audio(user,message.audio.file_id,caption=message.caption,reply_markup=keyboard)
                    else:
                        await bot.send_audio(user,message.audio.file_id,reply_markup=keyboard)
                elif message.content_type == 'video':
                    if message.caption:
                        await bot.send_video(user,message.video.file_id,caption=message.caption,reply_markup=keyboard)
                    else:
                        await bot.send_video(user,message.video.file_id,reply_markup=keyboard)
                elif message.content_type == 'sticker':
                    await bot.send_sticker(user,message.sticker.file_id,reply_markup=keyboard)
                elif message.content_type == 'voice':
                    await bot.send_voice(user,message.voice.file_id,reply_markup=keyboard)
                elif message.content_type == 'video_note':
                    await bot.send_video_note(user,message.video_note.file_id,reply_markup=keyboard)
                elif message.content_type == 'animation':
                    if message.caption:
                        await bot.send_animation(user,message.document.file_id,caption=message.caption,reply_markup=keyboard)
                    else:
                        await bot.send_animation(user,message.document.file_id,reply_markup=keyboard)
                # Se não hover erro encerra o loop de tentativas
                statistics['sucess'] += 1
                break
                ...
                # Se o erro for outro qualquer, que não seja do Telegram, etc encerra as tentativas
...
@bot.inline_handler(lambda query: re.match(inline_query_regex, query.query.replace('\n', ' ')))
async def inline_query_hide(inline_query: types.InlineQuery):
    try:
        ...
        if len(body) > 200:
            await bot.answer_inline_query(inline_query.id,[rsc.query_results.message_too_long(target.language_code)])
            return
        ...
        logger.warning(
            ...
            ' com carga útil: "' + inline_query.query + '"')
...
async def inline_query_spoiler(inline_query: types.InlineQuery):
    try:
        ...
        await bot.answer_inline_query(inline_query.id,[rsc.query_results.mode_spoiler(target.language_code, post.get_id(), body)])
    except Exception as e:
        logger.error(e)
        logger.warning(
    ...
    try:
        if call.data == "donate_button":
            ...
            await bot.edit_message_text(
                    ...
                    parse_mode = 'HTML',
                    reply_markup = rsc.keyboards.info_keyboard(lang=call.from_user.language_code),
                    disable_web_page_preview = True)
        else:
            ...
            if post.can_be_accessed_by(call.from_user, PostMode[mode]):
                ...
                await bot.answer_callback_query(call.id, post.content
                ...
                await bot.answer_callback_query(call.id,locales[call.from_user.language_code].not_allowed, True)
    except Exception as e:
        logger.error(e)
        logger.warning(
            'não pode lidar com a consulta de retorno de chamada de ' +
            get_formatted_username_or_id(call.from_user) +
            ' com carga útil: "' + call.data + '"')
        ...
        if str(message.chat.id) in admins:
            if message.text.startswith("/sudo "):
                ...
                try:
                    ...
                    await bot.reply_to(message, f"Erro: ID de usuário incorreto!")
                    return
                else:
                    await bot.reply_to(message, f"User {user_to_elevate} now is sudo!")
                ...
                try:
                    ...
                    await bot.reply_to(message, f"User {user_to_unelevate} now is no sudo!")
            else:
                await bot.reply_to(message, f"Você deve informar um ID como argumento!")
        else:
        ...
        if str(message.chat.id) in admins or target.has_sudo :
            if message.text.startswith("/ban "):
                ...
                try:
                    ...
                    await bot.reply_to(message, f"User {user_to_ban} now is banned!")
                ...
                try:
                    ...
                    await bot.reply_to(message, f"User {user_to_unban} now is unbanned!")
            else:
                await bot.reply_to(message, f"Você deve informar um ID como argumento!")
        else:
            await bot.reply_to(message, f"Acesso negado!")
    ...
    if message.chat.type == 'private':
        ...
        if message.text.startswith('/start '):
            ...
            if cmd_arg == 'how_to_use':
                await bot.reply_to(message, locales[message.from_user.language_code].how_to_use_link)
                return
        await bot.reply_to(message,
                text = locales[message.from_user.language_code].info_message,
                parse_mode = 'html',
                reply_markup = rsc.keyboards.info_keyboard(lang=message.from_user.language_code),
                disable_web_page_preview = True)
    else:
    ...
    if message.chat.type == 'private':
        ...
        if str(message.chat.id) in admins or target.has_sudo :
            if message.text.startswith('/bcusers'):
                await bot.send_message(message.chat.id, broadcast_text.format('users'), reply_markup=types.ForceReply(input_field_placeholder="Broadcast users..."))
            else:
                await bot.send_message(message.chat.id, broadcast_text.format('grupos'), reply_markup=types.ForceReply(input_field_placeholder="Broadcast groups..."))
...
async def all_messages(message: types.Message):
    try:
            ...
            if message.reply_to_message.text in [broadcast_text.format('users'),broadcast_text.format('grupos')]:
                ...
                if message.content_type in type_permited:
                    ...
                    if message.reply_to_message.text == broadcast_text.format('users'):
                        target_type = 'users'
                        # thread = Thread(target=runThreadSendMsg, args=(message,'users',))
                        # thread.start()
        ...
        logger.error(e)
        logger.warning('não é possível enviar informações para chat_id=' + str(message.chat.id))
       
@bot.my_chat_member_handler()
async def send_group_greeting(message: types.ChatMemberUpdated):
    old = message.old_chat_member
    new = message.new_chat_member
    if message.chat.type != "private" and new.status in ["member","administrator"]:
        try:
            target = Group.get_or_create(message.chat)
            if target.has_dialog:
                target.save()
            ...
            await bot.send_sticker(group_id, rsc.media.group_greeting_sticker_id())
            await bot.send_message(
                group_id,
